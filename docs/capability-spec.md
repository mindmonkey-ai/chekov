...
Rulings 2026-09-15 (approved, human approval in chat: "I approve"): the three follow-ups of the tool-use lane receipt. (1) The forced arm's cap is `FORCED_MAX_TOKENS` = the tool cap (1024) and rides in the agentic identity beside the other three caps, so runs under the ruling refuse to compare with, or resume, the `7882af966fae` runs; the grammar axis was blind on any thinker at 256. (2) A `tool_loop` row records `calls`, the tool names in order (never the arguments, never scored); rows written before the field load with none, and the failure line prints `(calls: …)` after the stop reason. (3) `chekov pull` copies each shard through a watched reader and gives it up as `TimedOut` after `[pull] stall_timeout_secs` (default 180; 0 refused at load) of silence, `.part` kept for resume — ureq 3 has no per-read timeout and its body budget is a total no 30 GB shard can use. Shipped 2026-09-15; the re-measurement under the new identity is pending.
