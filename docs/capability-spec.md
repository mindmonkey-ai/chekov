...
|---|---|---|---|
| `tool_emit` | 39 | Native tool-call emission, unconstrained decoding | BFCL-style AST match on the `tool_use` block: name + sorted arg keys + values vs a golden record via `serde_json`. Includes **30 calls** and **9 abstentions**: two requests needing no tool, four unavailable capabilities, and three requests missing required information |
| `grammar_gap` | 30 | The 30 call-expected tool cases with `json_schema` forced, buffered only. **`gap = forced − unconstrained`** | The single best anti-self-deception device in any of the three designs: measuring only with the grammar on makes every model score ~100%, and **Claude Code does not force grammars**. A large gap means "works only with a babysitter" |
...
Status 2026-09-10: `tool_loop` shipped — see `docs/superpowers/specs/2026-09-06-tool-loop-probe-design.md`. `long_ctx_trace` is implemented as the opt-in `--long-ctx-trace LENGTHS` option approved in `IDEAS.md`: four placements per requested length on both transports, a seeded synthetic code corpus, exact visible-answer grading, and a printed-only context recommendation. Long-context traces remain opt-in. Managed servers' templated prompt counts are checked before inference and compared with response usage; unverified lengths, including foreign-runtime lengths, cannot support a recommendation. At four cases, the ≥90% threshold requires all four to pass on both transports throughout the contiguous tested range. The recommended context includes the answer reserve and is a tested lower bound, never a discovered maximum. Live acceptance passed on Ornith: all 32 crossings at requested lengths 4096, 16384, 65536, and 131072 returned exact answers with verified token counts; the measured run and acceptance receipt are recorded in `IDEAS.md`. Still deferred: `diff_fidelity`, `think_leak` (§13 Q5), `hallucination` (largely covered by codebase tier 5).

Agentic corpus expansion 2026-09-10: the approved 39/30/40 set appends 23 call cases, 6 abstentions, and 28 instruction cases while preserving every old question and all six tool-loop scenarios. There are 188 single-turn crossings per model plus the existing loop-turn ceiling. The existing content hash identifies the new workload; old/new agentic comparison and resume refuse, and throughput-only hashes are unchanged. Automated validation passes. The 2026-09-11 UTC campaign completed all 600 crossings on the frozen `e0d71495afd7` corpus, using isolated server state without interrupting another session. Qwen / Ornith / GPT-OSS scored 9/40 / 29/40 / 39/40 on strict instructions, 36/39 / 35/39 / 35/39 on tools, 29/30 / 29/30 / 27/30 on forced grammar, and 6/6 / 6/6 / 5/6 on loops; pass/fail agreed across transports. Of 115 distinct axis/case pairs, 45 distinguish the configured stacks and 70 pass everywhere. Context, quantization, MTP, and reasoning settings differ, so this is not a controlled model-only ranking. Empty visible-answer passes and budget-related failures motivate a proposed observability follow-up; no cases or grades were changed. See the approved decision and [campaign receipt](../IDEAS.md#three-model-campaign-preflight-2026-09-11-utc) for per-case findings, limitations, validation, and the raw archive.

Ruling 2026-09-13 (approved): the follow-up found the instruction gap was mostly the 512-token reply cap starving long thinkers — Qwen's 32 empty instruction rows all sat at the thinking ceiling, and with 4096 tokens its strict score was 28/40. Every graded agentic row now records the crossing's `stop_reason` and the reports print it; instruction probes get 4096 tokens and tool probes 1024 (the forced arm keeps 256); an instruction reply with no visible text fails strict and loose, and a textless abstention fails. The caps and a grader version ride in the agentic prompt-set hash, so runs under the ruling refuse to compare with, or resume, the 2026-09-11 campaign, which stays untouched as evidence. Re-measured under the new identity (`ab4cd9b077d2`) on 2026-09-14: Qwen / Ornith / GPT-OSS scored 27/40 / 32/40 / 36/40 on strict instructions, 37/39 / 36/39 / 35/39 on tools, 29/30 / 29/30 / 28/30 on forced grammar, and 6/6 / 6/6 / 3/5 on loops (one GPT-OSS loop case unavailable to an engine-side parse error); 32 of 114 axis/case pairs distinguish the configured stacks and 81 pass everywhere, against 45 of 115 and 70 before. `compare` refuses the old runs against the new, naming `prompt_set_hash`. Per-case flips, the remaining `max_tokens` runaways, and timings are in [the ruling entry](../IDEAS.md#stop-reason-on-graded-agentic-rows-2026-09-13).

Loop expansion 2026-09-14 (approved): the loop axis was saturated under the ruling (two stacks at 6/6, five of six loops passing everywhere), so six cases `tl-007`–`tl-012` join the set with the seed six byte-identical — a decoy grep hit before the definition, the `edit_file` uniqueness contract, a test result that contradicts the prompt, an already-done change, a three-file chain, and two edits behind one test gate. The goal schema gains an optional `contains_all` beside `contains_any` (exactly one is set; `run_tests` reports `ok` only when every string landed); the TOML version stays 0. A scripted competent agent closes every added case in six turns or fewer in the unit tests. Twelve loops change the agentic identity again by construction. Measured 2026-09-14 under identity `57c7585512ec`: loops 12/12 / 10/12 / 7/11 for Qwen / Ornith / GPT-OSS (one GPT-OSS case unavailable to the same engine-side parse error), five of twelve loop cases separating the stacks against two of six before, every single-turn verdict identical to the ruling run; 35 of 120 axis/case pairs distinguish. See the [decision](../IDEAS.md#harder-tool-loop-scenarios-loop-corpus-expansion-decision-2026-09-14) for per-case findings. Loop-turn cap ruling 2026-09-14 (approved): `loop_probe` sends 4096 tokens per turn, the instruction cap, and that cap rides in the identity hash. Measured under identity `7882af966fae`: exactly one of 636 verdicts changed against the twelve-loop run (Ornith `tl-011` closes; loops 12/12 / 11/12 / 7/11), no loop turn stops on `max_tokens`, and 34 of 120 axis/case pairs distinguish. The receipt is in the same entry.

Tool-use lane measurement 2026-09-15 (approved 2026-09-09): Muse Glimmer 30B (Q8_0, Meta's sampling) and Nemotron 3.5 Lightning 30B-A3B (Q8_0, NVIDIA's sampling, one native MTP head) ran `--suite agentic` under the loop-cap identity `7882af966fae` and are read against Ornith's run on it. Strict instructions 39/40 / 37/40 against Ornith's 32/40; tools 32/39 / 34/39 against 36/39; loops 8/12 / 8/12 against 11/12. Both candidates exhaust the same four loop cases (`tl-003`, `tl-005`, `tl-009`, `tl-012`: the three test-gated edits and the missing-file abstention) and pass the decoy grep hit Ornith fails. Of 92 axis/case pairs graded on all three, 25 distinguish and 65 pass everywhere; five of twelve loop cases separate. Muse's forced grammar arm is unavailable on 29 of 30 cases (the engine's peg-native parse error, HTTP 500) and reads N/A; Nemotron's 2/30 is the forced arm's 256-token cap, which is not in the identity hash, starving a model that thinks ~900 characters under the grammar — 28 of 30 forced replies stopped on `max_tokens` with no answer. Muse's first run (1/40 strict) measured a registry mistake: `--reasoning-format none` leaves its `to=self` reasoning inline because the engine's Muse Glimmer parser only extracts under the default format; the rerun with no reasoning flag is the measurement. Muse is the first stack whose verdicts differ across transports (two of 91 cases). Raw evidence with a SHA-256 manifest: `docs/agentic-campaign-20260915-tooluse-lane.tar.gz`.

Rulings 2026-09-15 (approved, human approval in chat: "I approve"): the three follow-ups of the tool-use lane receipt. (1) The forced arm's cap is `FORCED_MAX_TOKENS` = the tool cap (1024) and rides in the agentic identity beside the other three caps, so runs under the ruling refuse to compare with, or resume, the `7882af966fae` runs; the grammar axis was blind on any thinker at 256. (2) A `tool_loop` row records `calls`, the tool names in order (never the arguments, never scored); rows written before the field load with none, and the failure line prints `(calls: …)` after the stop reason. (3) `chekov pull` copies each shard through a watched reader and gives it up as `TimedOut` after `[pull] stall_timeout_secs` (default 180; 0 refused at load) of silence, `.part` kept for resume — ureq 3 has no per-read timeout and its body budget is a total no 30 GB shard can use. Shipped 2026-09-15 and re-measured the same night under identity `e4cdb862d9e6` (five stacks, 1060 rows, runs `20260915T050243Z`…`20260915T065124Z`): Nemotron's forced arm 2/30 → 25/30 with every other Nemotron verdict reproduced; Ornith, GPT-OSS and Qwen reproduced all 212 verdicts each; Muse changed two — its earlier transport asymmetries now pass buffered — on requests the cap does not touch, so Muse at temperature 1.0 is the first stack whose seeded sampling does not reproduce run to run. The loop traces show Nemotron re-running `run_tests` without editing and Muse re-reading without editing or stopping; neither is the turn budget. The tool-use verdict stands.
