...
- Edition 2024 makes `std::env::set_var`/`remove_var` **unsafe**. Every test that points `$CHEKOV_CARGO` at a fake cargo therefore wraps the two calls in `unsafe { … }`, holds the module's `CARGO_ENV` mutex across the whole test (the variable is process-wide and the suite runs threaded), and carries a one-line comment saying the mutex is what makes the write sound. `#[allow]` is not available to silence anything here — the blocks are genuinely unsafe and the mutex is the reason they are correct.
