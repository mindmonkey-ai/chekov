//! Direct behavioural tests for the domain. These grade a candidate's *known*
...
fn from_str_is_exact_and_apply_is_arithmetic() {
    assert_eq!(apply(CreditCommand::Credit(from_str("10.00").unwrap()), Cents::default()), Cents(1000));
    assert_eq!(apply(CreditCommand::Debit(from_str("2499.95").unwrap()), Cents::default()), Cents(-249995));
}
